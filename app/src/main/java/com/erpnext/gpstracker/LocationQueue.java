...
final class LocationQueue extends SQLiteOpenHelper {
    static final class Item {
        ...
        Item(long id, String payload) { this.id=id; this.payload=payload; }
    }

    LocationQueue(Context context) { super(context,"bluecore_locations.db",null,1); }

    @Override public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE pending_locations (id INTEGER PRIMARY KEY AUTOINCREMENT, payload TEXT NOT NULL, created_at INTEGER NOT NULL, attempts INTEGER NOT NULL DEFAULT 0, last_error TEXT)");
    ...
    synchronized long enqueue(String payload,long createdAt) {
        ...
        values.put("payload",payload); values.put("created_at",createdAt);
        return getWritableDatabase().insertOrThrow("pending_locations",null,values);
    }
    synchronized Item oldest() {
        try(Cursor cursor=getReadableDatabase().query("pending_locations",new String[]{"id","payload"},null,null,null,null,"created_at ASC, id ASC","1")) {
            return cursor.moveToFirst()?new Item(cursor.getLong(0),cursor.getString(1)):null;
        }
    }
