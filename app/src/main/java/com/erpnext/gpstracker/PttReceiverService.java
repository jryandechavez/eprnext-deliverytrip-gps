...
public class PttReceiverService extends Service {
    ...
    private Notification notification(String text){Intent open=new Intent(this,MainActivity.class);PendingIntent p=PendingIntent.getActivity(this,0,open,PendingIntent.FLAG_UPDATE_CURRENT|(Build.VERSION.SDK_INT>=23?PendingIntent.FLAG_IMMUTABLE:0));Notification.Builder b=Build.VERSION.SDK_INT>=26?new Notification.Builder(this,CHANNEL):new Notification.Builder(this);return b.setSmallIcon(R.mipmap.ic_launcher).setContentTitle("Tic & Terry Fleet Radio").setContentText(text).setOngoing(true).setContentIntent(p).build();}
    private void poll(){if(!running)return;if(!Config.sessionId(this).isEmpty()||!Config.key(this).isEmpty())new Thread(()->{try{JSONObject result=request(new JSONObject().put("after",cursor).put("device_id",Config.deviceId(this)));cursor=result.optString("cursor",cursor);JSONArray rows=result.optJSONArray("messages");if(rows!=null)for(int i=0;i<rows.length();i++){JSONObject m=rows.getJSONObject(i);try{play(m.getString("audio"),m.optString("sender"));}catch(Exception e){Log.e("BluecorePTT","Background playback failed",e);}}}catch(Exception e){Log.e("BluecorePTT","Background polling failed",e);}finally{handler.postDelayed(this::poll,2000);}}).start();else handler.postDelayed(this::poll,2000);}
